...
banco de dados, via pymongo (síncrono). Toda operação de leitura/escrita
exige `owner_id` e embute esse filtro diretamente na query — nunca busca por
ID e confere o dono depois em Python (ver D1 em
...
class CardRepository(ICardRepository):
    ...
    def save_many(self, cards: List[Card]) -> List[Card]:
        if not cards:
            return []
    ...
    def find_by_id(self, card_id: uuid.UUID, owner_id: str) -> Optional[Card]:
        try:
            collection = self._get_collection()
    ...
    def find_by_front(self, front: str, owner_id: str) -> List[Card]:
        try:
            collection = self._get_collection()
    ...
    def find_by_deck_id(self, deck_id: uuid.UUID, owner_id: str) -> List[Card]:
        try:
            collection = self._get_collection()
            cursor = collection.find(
                {
    ...
    def find_by_context(self, context: str, owner_id: str) -> List[Card]:
        try:
            collection = self._get_collection()
    ...
    def find_duplicates(self, card: Card, owner_id: str) -> List[Card]:
        try:
            collection = self._get_collection()
    ...
    def delete(self, card_id: uuid.UUID, owner_id: str) -> bool:
    ...
    def delete_by_deck_id(self, deck_id: uuid.UUID, owner_id: str) -> int:
    ...
    def count(self, owner_id: str) -> int:
        try:
            collection = self._get_collection()
            return collection.count_documents(base_filter(owner_id))

        except Exception as e:
            raise RepositoryError(f"Failed to count cards: {e}")

    def count_by_deck_id(self, deck_id: uuid.UUID, owner_id: str) -> int:
        try:
            collection = self._get_collection()
    ...
    def exists(self, card_id: uuid.UUID, owner_id: str) -> bool:
        try:
            collection = self._get_collection()
