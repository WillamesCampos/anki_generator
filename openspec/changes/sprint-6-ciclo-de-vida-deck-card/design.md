...
Dois gaps de infraestrutura encontrados só ao testar de ponta a ponta (não bastava registrar `CELERY_BEAT_SCHEDULE` e assumir que funcionava):
- **`CELERY_BEAT_SCHEDULE` sozinho não agenda nada** — só existia o serviço `celery-worker` (consome fila) no `docker-compose.yml`; sem um processo `celery beat` (scheduler) rodando, ninguém lê essa configuração e dispara a task. Adicionado o serviço `celery-beat`, com volume nomeado próprio (`celery_beat_data:/var/lib/celery`, mesmo padrão do `document_generator_audio`) — o `--schedule` (arquivo de estado do scheduler) precisa ficar fora de `/app`, porque o bind mount de dev sobrescreve o `chown` feito na imagem, e o usuário não-root não conseguia escrever ali.
- **`schedule: timedelta(days=1)` vs `crontab(hour=0, minute=0)`**: `timedelta` é um intervalo relativo (roda 24h depois de o `celery-beat` ter iniciado, deriva se o container reiniciar) — decisão explícita do usuário: horário fixo, meia-noite. Trocado pra `crontab(hour=0, minute=0)`.
- **Fuso do `crontab`**: `TIME_ZONE` do Django é `UTC` (grava tudo em UTC no banco — boa prática, independente de onde o usuário está), mas isso faria a `crontab` avaliar "meia-noite" em UTC, não Brasília — decisão explícita do usuário: precisa respeitar o fuso real. `CELERY_TIMEZONE = "America/Sao_Paulo"` (novo, `core/settings/base.py`) é lido por `app.config_from_object(..., namespace="CELERY")` e só afeta como o Celery avalia `crontab(...)` — não muda como timestamps são armazenados no banco. Confirmado nos logs reais do `celery-beat` (`"Reset: Timezone changed from 'UTC' to 'America/Sao_Paulo'"`) e via `app.conf.timezone`/`app.now()`.
