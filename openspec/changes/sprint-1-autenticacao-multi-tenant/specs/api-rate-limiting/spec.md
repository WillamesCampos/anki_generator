...
### Requirement: Per-user/client rate limiting at 10 requests/second
The system SHALL throttle API requests to 10 requests/second per authenticated user/client, using DRF's built-in throttle classes backed by the Redis cache already configured in Sprint 0.

#### Scenario: Requests within the limit succeed
- **WHEN** a user/client sends requests at or below 10 requests/second
- **THEN** all requests are processed normally

#### Scenario: Requests over the limit are rejected
- **WHEN** a user/client exceeds 10 requests/second
