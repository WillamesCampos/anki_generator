...
- [x] 7.10 `Sidebar.jsx`/`Sidebar.css`: adotar ícones nomeados do `lucide-react` em todos os itens e no toggle; no modo recolhido, mostrar somente ícones com nomes acessíveis completos e tooltips no hover/foco, sem alterar rotas, persistência, dimensões ou adicionar sombra ao toggle
- [x] 7.11 `StudySessionPage.jsx`/`StudySessionPage.css`: enriquecer o card branco com borda preta, sombra laranja e conteúdo centralizado; aplicar tokens semânticos vermelho/âmbar/azul/verde aos botões Errei/Difícil/Bom/Fácil em grade responsiva, preservando valores, ordem, API e estados de interação
- [x] 7.12 `HomePage.css`: corrigir a sombra dos dois cards de resumo para o mesmo preto deslocado `4px 4px 0` do gráfico, removendo a sobrescrita laranja específica da meta e cobrindo o contrato visual com teste automatizado
