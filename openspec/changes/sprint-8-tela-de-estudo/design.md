...
- **Alternativa descartada**: reconsultar `find_due` a cada avaliação, ao invés de usar a lista buscada no início. Rejeitada — geraria uma chamada de API extra por card revisado (contra o orçamento de rate limit de 3 req/s já estabelecido no projeto), pra resolver um caso (reaparecimento de "again" na mesma sessão) que a decisão de produto já aceitou não cobrir nesta v1.
