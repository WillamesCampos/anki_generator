...
- [x] **Autenticação service-to-service via JWT** (Sprint 5, movida da Sprint 9): JWT HS256 auto-assinado entre Django e `document-generator`, segredo por par emissor↔verificador, rotação via `kid` versionado (sem secrets manager), verificação local sem cache/introspection — ver `autenticacao-service-to-service-jwt`.
