...
<decisao_resolvida id="recuperacao-de-senha-resend">
Sprint 9 (ampliada, a pedido do usuário — fecha os itens 1 e 4 de `PRD.md` §7.1): fluxo de "esqueci minha senha", desbloqueado por um `EMAIL_BACKEND` que nunca tinha sido configurado (fora do escopo até aqui). Levantamento de provedores via `backend-mentor`: descartado self-host de SMTP (deliverability depende de reputação de IP/domínio, não é um problema que se resolve com mais esforço de configuração — exceção real à regra geral de "self-host primeiro" deste projeto); descartado Amazon SES (traria AWS de volta pro backend, decisão já tomada de manter fora); descartados SendGrid/Mailgun (perderam o tier grátis permanente) e Postmark (cobra desde o primeiro e-mail). Escolhido **Resend** (3.000 e-mails/mês grátis, feito especificamente pra transacional) sobre Brevo (grátis também, mas suíte de marketing+transacional, superfície maior que o necessário).

`django-anymail` foi considerado (unificaria a API HTTP do Resend atrás da interface `EMAIL_BACKEND` do Django, com webhooks de entrega/abertura/clique normalizados) mas descartado por ora — decisão explícita do usuário: sem domínio próprio verificado ainda, webhooks não são testáveis de ponta a ponta (exigem URL pública alcançável), e o relay SMTP nativo do Resend já resolve o caso de uso atual (só recuperação de senha) sem a dependência extra. Revisitar quando o deploy real (Sprint 15) e o domínio existirem.

Implementação: `EMAIL_BACKEND` de console em dev (`core/settings/local.py`), relay SMTP do Resend em produção (`core/settings/production.py`, credencial via `RESEND_API_KEY`, nunca hardcoded). `apps/accounts/serializers.py` (`CustomPasswordResetSerializer`) customiza só o `url_generator` do fluxo do `dj-rest-auth`/`allauth` — como `django-allauth` está instalado, `dj-rest-auth` delega pro `AllAuthPasswordResetForm`, que gera o link via `reverse("password_reset_confirm")` por padrão (uma URL Django que não existe nesta arquitetura API-only); o override aponta o link pro frontend (`FRONTEND_URL/redefinir-senha?uid=...&token=...`) usando a codificação de `uid` do allauth (base36 pra PK inteira, não o base64 do Django puro). **Decisão revisada na Sprint 9**: o e-mail passa a sobrescrever nativamente `account/email/password_reset_key` em assunto, texto puro e HTML “Dark premium”. O diretório de templates do projeto tem precedência sobre `APP_DIRS`; não há adapter ou formulário customizado, recursos externos, alteração de token, URL da SPA ou telas React.

Corrigido no caminho: o `Site` (`django.contrib.sites`, `SITE_ID=1`) nunca tinha sido configurado desde o Sprint 0/1, aparecendo literalmente como "example.com" no e-mail — migration de dados corrige pra "Anki Generator" (domínio provisório `localhost:8000`, até existir domínio próprio). A correção exigiu `update_or_create` em vez de `filter().update()`: a linha default do `Site` é criada por um signal `post_migrate` do Django, que roda depois de todas as migrations — um `update()` simples seria um no-op silencioso (a linha ainda não existe nesse momento) e o signal recriaria "example.com" logo em seguida.

Frontend: páginas novas `/esqueci-minha-senha` (solicita o e-mail, sempre mostra a mesma mensagem de sucesso, com ou sem erro — não revela se a conta existe) e `/redefinir-senha` (lê `uid`/`token` da URL, formulário de nova senha). Testado de ponta a ponta com a API key real do Resend: e-mail entregue com sucesso na caixa de entrada do usuário.
</decisao_resolvida>
